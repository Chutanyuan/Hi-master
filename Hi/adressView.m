...
- (void)buttonClick:(UIButton *)sender
{
    ...
    if (sender.tag==2) {
        ...
        [self.delegate adressLabelText:titlelabel.text];
    }
}

-(void)setAdressArray:(NSArray *)adressArray
{
    int line;
    if (adressArray.count<12) {
        line = 4;
    }else{
        double lineNumber = adressArray.count/4.0;
        line  = ceil(lineNumber);
    }
    ...
    [adressArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        label.text = [NSString stringWithFormat:@"%@",[objectBM objectForKey:@"departId"]];
